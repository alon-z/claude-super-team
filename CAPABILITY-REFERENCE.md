...
|---|---|---|---|---|
| `name` | Unique skill identifier (lowercase, hyphens, max 64 chars) | In use | Skills System | Used in all 13 skills |
| `description` | When to use; drives auto-invocation | In use | Skills System | Used in all 13 skills |
...
| `allowed-tools` / `tools` | Tool access allowlist | In use | Skills System | Used in all 13 skills |
| `disallowedTools` | Tool denylist | Documented but unused | Skills System | Alternative to allowlist; useful when most tools needed |
| `model` | Override model (sonnet/opus/haiku) | Documented but unused | Skills System | Skills currently rely on agent model selection |
| `context` | `skill` (default) or `fork` (spawns subagent) | Documented but unused | Skills System | All skills currently use default `skill` context |
...
| Skill tool invocation | `/skill-name` slash commands | In use | Skills System | Primary invocation method for all 13 skills |
...
| Skill precedence | Enterprise > personal > project; skill wins over built-in command | Documented but unused | Not covered | Relevant if name conflicts arise |
| Description character budget | `SLASH_COMMAND_TOOL_CHAR_BUDGET` env var; default 2% of context window | Documented but unused | Not covered | Impacts how many skills Claude can see at once |

**Expanded notes on items not in ORCH-REF:**
...
| `Explore` | Fast read-only codebase exploration (Haiku) | Documented but unused | Custom Agents | Lightweight; good for `context: fork` skills |
...
| `--agent NAME` CLI flag | Run entire session as specific agent | Documented but unused | CLI Flags | Useful for specialized sessions |
| `--agents JSON` CLI flag | Define agents inline (session-only) | Documented but unused | CLI Flags | Ephemeral agent definitions |

**Expanded note -- Agent memory**: When `memory: project` is set, the agent stores learnings in `.claude/agents/{name}/MEMORY.md`. First 200 lines are included in the agent's system prompt on subsequent invocations. Three scopes: `user` (cross-project), `project` (version-controllable), `local` (gitignored). Source: code.claude.com/docs/en/sub-agents#supported-frontmatter-fields.
...
| Capability | Description | Status | ORCH-REF Section | Notes |
|---|---|---|---|---|
| `SessionStart` | Fires on startup/resume/clear/compact | Documented but unused | Hooks System | High value: inject context after compaction |
...
| Capability | Description | Status | ORCH-REF Section | Notes |
|---|---|---|---|---|
| Skills | Slash command definitions | In use | Plugins System | 13 skills across 3 plugins |
...
**Expanded note -- Output Styles**: Output styles customize Claude's system prompt to change response format and behavior. Files use markdown with `name`, `description`, `keep-coding-instructions` (boolean) frontmatter. Shipped via plugin `outputStyles` directory, activated via `/output-style` command or `outputStyle` setting. Predefined styles include Explanatory, Learning, and others. Source: code.claude.com/docs/en/output-styles.

**Expanded note -- LSP Servers**: Plugins can bundle Language Server Protocol configurations via `.lsp.json`. Provides real-time code intelligence: diagnostics, go-to-definition, find-references, hover info. The LSP tool is the client-side interface. Source: code.claude.com/docs/en/plugins-reference#lsp-servers.
...
**Tool naming for MCP**: MCP tools use `mcp__<server>__<tool>` naming. Hook matchers can use regex patterns like `mcp__.*__write.*`. Wildcard permissions: `mcp__server__*`. See ORCH-REF "MCP Servers" section.
...
| `--disallowedTools TOOLS` | Deny specific tools | Documented but unused | CLI Flags |
...
| `--resume SESSION_ID` | Resume specific session | Documented but unused | CLI Flags |
| `--fork-session` | Create divergent branch on resume | Documented but unused | CLI Flags |
| `--session-id UUID` | Use specific session ID | Documented but unused | CLI Flags |
| `--from-pr NUMBER` | Resume from GitHub PR | Documented but unused | CLI Flags |
...
| `--max-turns N` | Turn limit | Documented but unused | CLI Flags |
...
| Flag | Description | Status | ORCH-REF Section |
|---|---|---|---|
| `--system-prompt TEXT` | Replace entire prompt | Documented but unused | CLI Flags |
| `--system-prompt-file PATH` | Load from file | Documented but unused | CLI Flags |
| `--append-system-prompt TEXT` | Add to default | Documented but unused | CLI Flags |

### Configuration

| Flag | Description | Status | ORCH-REF Section |
|---|---|---|---|
| `--mcp-config FILE` | Load MCP servers | Documented but unused | CLI Flags |
| `--add-dir PATH` | Additional working directories | Documented but unused | CLI Flags |
| `--settings FILE` | Load settings from file | Documented but unused | CLI Flags |
...
| `--plugin-dir PATH` | Load plugin from local directory | Documented but unused | Not covered |
...
| Nested CLAUDE.md | Child directory CLAUDE.md loaded on demand | Documented but unused | Memory & Context | Loaded when Claude reads files in that directory |
...
**Expanded note -- Auto Memory**: Claude automatically saves learnings to `MEMORY.md` in project-scoped directories (`~/.claude/projects/{project}/`). Can also create topic-specific files. First 200 lines of MEMORY.md are loaded into the system prompt. Disable with `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`. Manage with `/memory` command. Source: code.claude.com/docs/en/memory#auto-memory.

**Expanded note -- CLAUDE.local.md**: Gitignored personal overrides placed alongside CLAUDE.md. Automatically added to `.gitignore`. Use for personal API keys, local paths, or style preferences that should not be committed. Source: code.claude.com/docs/en/memory#determine-memory-type.

**Expanded note -- Modular rules**: Place `.md` files in `.claude/rules/` with optional `paths` frontmatter containing glob patterns. Rules are loaded only when Claude works with matching files. User-level rules go in `~/.claude/rules/`. Source: code.claude.com/docs/en/memory#modular-rules-with-claude-rules.
...
| `--resume SESSION_ID` | Resume specific session | Documented but unused | Session Management | Full context restoration |
...
| `statusline-setup` agent | Built-in setup agent | Documented but unused | Custom Agents |
...
|---|---|---|---|---|
| `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` | Enable agent teams | In use | Agent Teams | Set to `1` |
...
| `CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR` | Reset cwd after each Bash | Documented but unused | Not covered | Set to `1` |
...
| `CLAUDE_CODE_REMOTE` | Set to `"true"` in remote web environments | Documented but unused | Not covered | |
...
| `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` | Load CLAUDE.md from --add-dir dirs | Documented but unused | Not covered | Set to `1` |
...
| Claude Agent SDK full API surface | ORCH-REF + training data | Install `@anthropic-ai/claude-agent-sdk`, verify `query()`, `canUseTool`, `--json-schema` |
...
| Phase 1 Research (01-RESEARCH.md) | `.planning/phases/01-claude-code-capability-mapping/` |
